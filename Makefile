...
# Builds minio locally for arm32.
build-arm32: checks
	@echo "Building minio binary to './minio'"
	@GO111MODULE=on CGO_ENABLED=0 GOOS=linux GOARM=7 GOARCH=arm go build -tags kqueue -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/minio 1>/dev/null
